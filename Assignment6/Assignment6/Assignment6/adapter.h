...
class objectAdapterStack : public Stack{
	...
	objectAdapterStack(DList &_dlist) {
		dlist = &_dlist;
	}
	...
	Object top() {
		DList::const_iterator iter = dlist->begin();
		return *iter;
	}
	bool isEmpty() {
		DList::const_iterator iterBeg = dlist->begin();
		if (*iterBeg == nullptr) {
			return true;
		}
		return false;
...
class classAdapterStack : public Stack, private DList {
public:
	void push(const Object& o) {
		push_front(o);
	}
